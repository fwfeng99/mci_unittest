...
class PipelineL1MCIAstrometry(BasePipelineInterface):
    ...
    def para_set(self) -> CsstResult:
        ...
        ins_output = dir_ins
        sci_c1_path = sci_path = "/nfsdata/share/pipeline-unittest/csst_mci/data/csst_mci_instrument/CSST_MCI_C1_EXDF_20230918003941_20230918004441_20100000001_07_L1_V01.fits"
    ...
    def retain_files_suffixes(self, suffixes=None) -> CsstResult:
        """
        Method, retain files with selected suffixes.

        Retain files with specified suffixes, if the suffixes list is empty or None, retain all files.
        ...
        Examples
        --------
        >>> result = case_astrometry.retain_files_suffixes(folder_path, suffixes=None)
        """

        dm = self._dm

        folder_path = dm.l1_file(name="gaiadr3nowlac.fits")

        # Create a set to store names of files that should be kept
        files_to_keep = set()
        if suffixes:
            # If a list of suffixes is provided, filter files based on these suffixes
                for suffix in suffixes:
                    # Iterate over the files in the folder and select those that match the suffixes
                    for file in os.listdir(folder_path):
                        if file.endswith(suffix):
                            files_to_keep.add(file)
        else:
            # If no suffix list is provided, add all files in the folder to the set
            files_to_keep = set(os.listdir(folder_path))

        # Iterate over all files in the folder
        for file in os.listdir(folder_path):
            if file not in files_to_keep:
                # If the file is not in the set of files to keep, delete it
                    os.remove(os.path.join(folder_path, file))
                    print(f"Deleted: {file}")
            else:
                # If the file is in the set of files to keep, retain it
