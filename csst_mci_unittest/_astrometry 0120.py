...
def update_catalog_space_motion(catalog, header, dfsapi: bool = False):
    """
    ...
    catalog_now = Table()
    catalog_now.add_column(
        Column(name="X_WORLD", data=c_epoch_now.ra.degree, unit=u.deg)
    )
    catalog_now.add_column(
        Column(name="Y_WORLD", data=c_epoch_now.dec.degree, unit=u.deg)
    )
    catalog_now.add_column(
        Column(name="pmRA", data=c_epoch_now.pm_ra_cosdec, unit=u.mas / u.yr)
    )
    catalog_now.add_column(
        Column(name="pmDE", data=c_epoch_now.pm_dec, unit=u.mas / u.yr)
    )
    catalog_now.add_column(
        Column(
            name="Plx",
            data=c_epoch_now.distance.to(u.arcsec, equivalencies=u.parallax()),
            unit=u.mas,
        )
    )
...
def run_mci_astrometry(dm: CsstMCIDataManager, logger: get_logger = None):
    ...
    # get the image fits
    img_path = dm.l1_detector(detector=dm.target_detectors[0], post="nimg.fits")
